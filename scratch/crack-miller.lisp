...
(defparameter *curr-params*
  `(:bus 16
    :dur 0.6
    :env ,(luftstrom::gen-env 0.1 0)
    :arrayidx 40
    :freq 400
    :lfofreq 10
    :amp 1
...
(defparameter *play-crack* t)
(defparameter *pending* nil)
...
;;               (format t "~a" d1)
               (case d1
                 ...
                 (1 (setf (getf *curr-params* :lfofreq) (cm::hertz (cm::interpl d2 '(0 -36 127 108)))))
                 (2 (setf (getf *curr-params* :delfreq) (cm::hertz (cm::interpl d2 '(0 -36 127 108)))))
                 (3 (setf (getf *curr-params* :arrayidx) (round (cm::interpl d2 '(0 0 127 100)))))
                 (4 (setf (getf *curr-params* :dur) (float (cm::interpl d2 '(0 0.01 127 3) :base 1))))
                 (5 (setf (getf *curr-params* :inner-dur) (float (cm::interpl d2 '(0 0.01 127 3) :base 300))))
                 (6 (setf (getf *curr-params* :rev-dur) (cm::interpl d2 '(0.01 0 127 3) :base 1)))
                 (7 (setf (getf *curr-params* :feedback) (cm::interpl d2 '(0 0.5 127 1))))
                 (16 (setf (getf *curr-params* :delprop) (cm::interpl d2 '(0 0.5 127 3))))
                 (17 (setf (getf *curr-params* :wet) (float (cm::interpl d2 '(0 0 127 1)))))
                 (18 (setf (getf *curr-params* :amp) (float (cm::interpl d2 '(0 0 127 1)))))
                 (19 (setf (getf *curr-params* :inner-suswidth) (cm::interpl d2 '(0 0 127 0.99))))
                 (20 (setf *dtime* (cm::interpl d2 '(0 0.01 127 1))))
                 (41 (play))
                 (22 (if *play-crack*
                         (progn
                           (apply #'bus-play-crack-lfo-rev *curr-params*)
                           (reschedule))
                         (setf *pending* (lambda () (apply #'bus-play-crack-lfo-rev *curr-params*)))))
;;                 (23 (setf (getf *curr-params* :dur) (cm::interpl d2 '(0.01 0 127 1))))
                 (t (if *play-crack*
                         (progn
                           (apply #'bus-play-crack-lfo-rev *curr-params*)
                           (reschedule))
