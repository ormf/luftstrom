...
(defun ensure-funcall (array synth key &rest args)
  (let ((fn (aref array (gethash key *audio-fn-idx-lookup*))))
    (if fn (apply fn args)
        (warn "no fn for ~S in ~S" key (aref array 0)))))
...
;;  (format t "~a ~%" tidx)
  (let* ((fndefs (fn-defs tidx))
         (p1 (ensure-funcall fndefs :p1 0 x y velo tidx))
         (p2 (ensure-funcall fndefs :p2 0 x y velo tidx p1))
         (p3 (ensure-funcall fndefs :p3 0 x y velo tidx p1 p2))
         (p4 (ensure-funcall fndefs :p4 0 x y velo tidx p1 p2 p3)))
    (let ((synth (ensure-funcall fndefs :synth 0 x y velo tidx p1 p2 p3 p4)))
      (case synth
        (0
         `(sc-user::sc-lfo-click-2d-bpf-4ch-out
          ,@(collect-args '(:pitch :amp)) :pitch (ensure-funcall fndefs :pitchfn x y velo tidx p1 p2 p3 p4)
          :amp (float (ensure-funcall fndefs synth :ampfn  x y velo tidx p1 p2 p3 p4))
          :dur (ensure-funcall fndefs synth :durfn x y velo tidx p1 p2 p3 p4)
          :suswidth (ensure-funcall fndefs synth :suswidthfn x y velo tidx p1 p2 p3 p4)
          :suspan (ensure-funcall fndefs synth :suspanfn x y velo tidx p1 p2 p3 p4)
          :decay-start (ensure-funcall fndefs synth :suspanfn x y velo tidx p1 p2 p3 p4)
          :decay-end (ensure-funcall fndefs synth :suspanfn x y velo tidx p1 p2 p3 p4)
          :lfo-freq (ensure-funcall fndefs synth :suspanfn x y velo tidx p1 p2 p3 p4)
          :x-pos (ensure-funcall fndefs synth :suspanfn x y velo tidx p1 p2 p3 p4)
          :y-pos (ensure-funcall fndefs synth :suspanfn x y velo tidx p1 p2 p3 p4)
          :wet (ensure-funcall fndefs synth :suspanfn x y velo tidx p1 p2 p3 p4)
          :filt-freq (ensure-funcall fndefs synth :suspanfn x y velo tidx p1 p2 p3 p4)
          :bp-freq (ensure-funcall fndefs synth :suspanfn x y velo tidx p1 p2 p3 p4)
          :bp-rq (ensure-funcall fndefs 19 x y velo tidx p1 p2 p3 p4)
          :head 200))
;;; (format t "~&args: ~%~S" args)
        (1
         (sc-user::sc-lfo-click-2d-bpf-vow-out
          :pitch (ensure-funcall fndefs 6 x y velo tidx p1 p2 p3 p4)
          :amp (float (ensure-funcall fndefs 7 x y velo tidx p1 p2 p3 p4))
          :dur (ensure-funcall fndefs 8 x y velo tidx p1 p2 p3 p4)
          :suswidth (ensure-funcall fndefs 9 x y velo tidx p1 p2 p3 p4)
          :suspan (ensure-funcall fndefs 10 x y velo tidx p1 p2 p3 p4)
          :decay-start (ensure-funcall fndefs 11 x y velo tidx p1 p2 p3 p4)
          :decay-end (ensure-funcall fndefs 12 x y velo tidx p1 p2 p3 p4)
          :lfo-freq (ensure-funcall fndefs 13 x y velo tidx p1 p2 p3 p4)
          :x-pos (ensure-funcall fndefs 14 x y velo tidx p1 p2 p3 p4)
          :y-pos (ensure-funcall fndefs 15 x y velo tidx p1 p2 p3 p4)
          :wet (ensure-funcall fndefs 16 x y velo tidx p1 p2 p3 p4)
          :filt-freq (ensure-funcall fndefs 17 x y velo tidx p1 p2 p3 p4)
          :bp-freq (ensure-funcall fndefs 18 x y velo tidx p1 p2 p3 p4)
          :bp-rq (ensure-funcall fndefs 19 x y velo tidx p1 p2 p3 p4)
          :voice-type (ensure-funcall fndefs 20 x y velo tidx p1 p2 p3 p4)
          :vowel (ensure-funcall fndefs 21 x y velo tidx p1 p2 p3 p4)
          :head 200))
