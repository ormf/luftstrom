...
(defmacro with-gl-cl-buffer ((vbo cl-buf context elt-size count) &body body)
  `(with-bound-buffer (:array-buffer ,vbo)
                      (%gl:buffer-data :array-buffer (* ,elt-size ,count) (cffi:null-pointer) :dynamic-draw)
                      (setf ,cl-buf
                            (create-from-gl-buffer ,context :read-write ,vbo))
                      ,@body
                      (format t "created shared GL buffer: ~a~%" ,cl-buf)))
...
;;;                :retrig-buffer retrig-buffer
      ...
      (unless (zerop gl-coords)
        (with-gl-cl-buffer (gl-obst-pos (obstacles-pos bs) *context* +float4-octets+ maxobstacles)
          (gl:with-mapped-buffer (p-obst-pos :array-buffer :read-write)
                  (loop repeat maxobstacles
                        for i from 0 by 2 ;;; as i indexes floats, we increase the float-count and *not* the byte-count!
                        do (progn
                             (setf (cffi:mem-aref p-obst-pos :float i) (float 100))
                             (setf (cffi:mem-aref p-obst-pos :float (1+ i)) (float 100))))))
...
(defun set-shared-buffers (bs context)
  (setf (obstacles-pos bs) (ocl:create-from-gl-buffer context :read-write (gl-obst-pos bs)))
  (setf (obstacles-lookahead bs) (ocl:create-from-gl-buffer context :read-write (gl-obst-lookahead bs)))
  (setf (obstacles-multiplier bs) (ocl:create-from-gl-buffer context :read-write (gl-obst-multiplier bs)))
  (setf (obstacles-type bs) (ocl:create-from-gl-buffer context :read-write (gl-obst-type bs)))
  (setf (obstacles-radius bs) (ocl:create-from-gl-buffer context :read-write (gl-obst-radius bs)))
  (setf (obstacles-boardoffs-maxidx bs) (ocl:create-from-gl-buffer context :read-write (gl-obst-boardoffs-maxidx bs))))
      ...
      (unless (zerop gl-coords)
        ...
        (with-gl-cl-buffer (gl-obst-radius (obstacles-radius bs) *context* +float-octets+ maxobstacles)
          (gl:with-mapped-buffer (p :array-buffer :read-write)
                  (loop repeat maxobstacles
                        for i from 0 ;;; as i indexes ints we increase the float-count and *not* the byte-count!
                        do (setf (cffi:mem-aref p :int i) 15))))
;;;        (set-obstacle-positions bs 120 110)
        (set-obstacle-positions bs 130 111)
        (with-gl-cl-buffer (gl-obst-lookahead (obstacles-lookahead bs) *context* +float-octets+ maxobstacles)
          (gl:with-mapped-buffer (p :array-buffer :write-only)
            (loop repeat maxobstacles
                  for i from 0 ;;; as i indexes floats we increase the float-count and *not* the byte-count!
                  do (setf (cffi:mem-aref p :float i) 2.5))))
        (set-obstacle-positions bs 120 105)
        (with-gl-cl-buffer (gl-obst-multiplier (obstacles-multiplier bs) *context* +float-octets+ maxobstacles)
          (gl:with-mapped-buffer (p :array-buffer :write-only)
            (loop repeat maxobstacles
                  for i from 0 ;;; as i indexes floats we increase the float-count and *not* the byte-count!
        ...
        (with-gl-cl-buffer (gl-obst-type (obstacles-type bs) *context* +float-octets+ maxobstacles)
          (gl:with-mapped-buffer (p :array-buffer :write-only)
            (loop repeat maxobstacles
                  for i from 0 ;;; as i indexes ints we increase the float-count and *not* the byte-count!
                  do (setf (cffi:mem-aref p :int i) 0))))
        (with-gl-cl-buffer (gl-obst-boardoffs-maxidx (obstacles-boardoffs-maxidx bs) *context* +float-octets+ maxobstacles)
          (gl:with-mapped-buffer (p :array-buffer :write-only)
            (loop repeat maxobstacles
                  for i from 0 ;;; as i indexes ints we increase the float-count and *not* the byte-count!
                  do (setf (cffi:mem-aref p :int i) 20))))


        (gl-enqueue (lambda () (set-shared-buffers bs *context*)))
                    ...
                    (progn
                      (loop repeat count
                            ...
                            do (let ((color (if (zerop i) *first-boid-color* *fg-color*)))
                                 ...
                                 (apply #'set-array-vals p (+ i 8) origin)
                                 (apply #'set-array-vals p (+ i 12) color)))))))
              (set-obstacle-positions bs 310 176)                
              (unless (zerop count)
                (set-obstacle-positions bs 210 176)                
                (with-gl-cl-buffer (gl-vel velocity-buffer *context* +float4-octets+ maxcount)
                  (gl:with-mapped-buffer (p-vel :array-buffer :write-only)
                    (unless (zerop count)
                      (progn
                        (loop repeat count
                              ...
                              for v = (float (+ 0.1 (random 0.1)) 1.0)
                              do (set-array-vals p-vel i (* v maxspeed (sin a))(* v maxspeed (cos a)) 0.0 0.0))))))
                (set-obstacle-positions bs 200 176)                
                (with-gl-cl-buffer (gl-life life-buffer *context* +float-octets+ maxcount)
                  (gl:with-mapped-buffer (p-life :array-buffer :write-only)
                    (unless (zerop count)
                      (dotimes (i count)
                ...
                ;;                                                                          (max 0.01 (* (+ 0.7 (random 0.2)) maxlife)))
                ;;                                                                      1.0))))
                (set-obstacle-positions bs 200 106)                
                (with-gl-cl-buffer (gl-retrig retrig-buffer *context* (* 4 2 +float-octets+) maxcount)
                  (gl:with-mapped-buffer (p-retrig :array-buffer :write-only)
                    (unless (zerop count)
                      (loop
                        ...
                        do (progn
                             ...
                             (setf (cffi:mem-aref p-retrig :long (+ i 3)) 0))))))
;;; board-offset initialization: write dx,dy,distance,sep and coh vectors
                (set-obstacle-positions bs 200 105)                
                (ocl:with-mapped-buffer (dx (car *command-queues*) board-dx max-offs-size :write t)
                  (ocl:with-mapped-buffer (dy (car *command-queues*) board-dy max-offs-size :write t)
                    (ocl:with-mapped-buffer (d (car *command-queues*) board-dist max-offs-size :write t)
                      (ocl:with-mapped-buffer (sep (car *command-queues*) board-sep (* 4 max-offs-size) :write t)
                          ...
                          (ocl:with-mapped-buffer (obst (car *command-queues*) obstacle-board-buffer max-offs-size :write t)
                            (loop
                              ...
                              do (progn
                                   ...
                                   (apply #'set-array-vals sep j vsep)
                                   (apply #'set-array-vals coh j vcoh)))))))))
                (set-obstacle-positions bs 230 160))))))
      bs)))
