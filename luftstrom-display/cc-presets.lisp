...
;;;                     (bp-set-value :maxforce (* speedf 0.09))
                ...
                ,(with-exp-midi-fn (5 250)
                   (bp-set-value :length (round (funcall ipfn d2))))
                (,player 14)
                ,(with-lin-midi-fn (0 50)
                   (bp-set-value :clockinterv (round (funcall ipfn d2)))))))
          (:nk-std-noreset ,(lambda (player)
                               `((,player 8)
...
(defun nk-std (player)
  ...
  `((,player 8)
    ,(with-exp-midi-fn (0.1 20)
       (let ((speed (float (funcall ipfn d2))))
         (bp-set-value :speed (dround speed))
;;         (bp-set-value :maxspeed (cl-boids-gpu::speed->maxspeed speed))
;;         (bp-set-value :maxforce (cl-boids-gpu::speed->maxforce speed))
         ))
    (,player 9)
    ,(with-lin-midi-fn (1 8)
       (bp-set-value :sepmult (float (funcall ipfn d2))))
...
(defparameter *tc1*
  (make-instance 'value-cell :ref (cl-boids-gpu::bp-speed *bp*)
                             :map-fn (ou:m-exp-fn 0.1 20)
                             :rmap-fn (ou:m-exp-rev-fn 0.1 20)))

(setf (val *tc1*) 0)

;;; (funcall (ou:m-exp-fn 0.1 20) 127)

(defun nk-std-noreset (player)
  `((,player 8)
    (,(with-exp-midi-fn (0.1 20)
        (let ((speed (float (funcall ipfn d2))))
          (bp-set-value :speed speed)
...
(defun boid-ctl1-noreset (player)
  `((,player 100)
    (list
     ,(with-exp-midi-fn (0.1 20)
        (unless (= (aref *cc-state* player 40) 127)
          (bp-set-value :speed (float (funcall ipfn d2)))
;;;            (bp-set-value :maxspeed (* speedf 1.05))
;;;            (bp-set-value :maxforce (* speedf 0.09))
...
(defun boid-ctl2 (player)
  `((,player 7)
    ,(with-exp-midi-fn (0.1 20)
       (unless (= (aref *cc-state* player 40) 127)
         (bp-set-value :speed (funcall ipfn d2))))
    (,player 70)
    ,(with-lin-midi-fn (1 8)
       (unless (= (aref *cc-state* player 40) 127)
         (bp-set-value :sepmult (float (funcall ipfn d2)))))
